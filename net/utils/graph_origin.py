...
class Graph():
    ...
    def get_edge(self, layout):
        ...
        elif layout == 'ted_gesture':
            self.num_node = 8
            self.self_link = [(i, i) for i in range(self.num_node)]
            self.inward = [(4, 3), (3, 2), (7, 6), (6, 5),
                             (5, 1), (2, 1),
                             (0, 1)]
            self.outward = [(j, i) for (i, j) in self.inward]
            self.neighbor_link = self.inward + self.outward
    ...
    def get_adjacency(self, strategy):
        if strategy == 'spatial':
            ...
            A[0] = tools.get_uniform_graph(self.num_node, self.self_link, self.neighbor_link)
            self.A = A
        elif strategy == 'spatial_all':
            A = tools.get_spatial_all_graph(self.num_node, self.self_link, self.inward, self.outward)
            self.A = A
        else:
