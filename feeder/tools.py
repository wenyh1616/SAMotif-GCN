...
def random_choose_TSN(data_numpy, size, auto_pad=True):
    # input: C,T,V,M
    C, T, V, M = data_numpy.shape
    if T == size:
        return data_numpy
        ...
        duration = T//size
        halfsize = int(size/duration)
        data_aug = np.zeros([C, halfsize, V, M * duration])
        for j in range(duration):
            startidx = j * size
            begin = random.randint(startidx,startidx + size - halfsize)
            # test = data_numpy[:, begin: begin + int(size/2), :, :]
            # data_aug[:, :, :, (j+1) * M - 2:(j+1) * M] = data_numpy[:, startidx: startidx + size, :, :]
            data_aug[:, :, :, j*M : (j+1) * M] = data_numpy[:, begin: begin + halfsize, :, :]
            # data_aug[:, :, :, :, j] = data_numpy[:, startidx: startidx + size, :, :]
        return data_aug
